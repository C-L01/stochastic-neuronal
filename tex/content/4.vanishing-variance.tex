...
\end{proposition}
\begin{proof}
  Recall that \((\ol{M}_s^{\lambda,N})_{s\in[0,T]}\) is a martingale by \zcref{prop:emp-M-mart}.
  In particular,
  \begin{equation}
  ...
  \end{equation}
  % \begin{equation}
  %     (\ol{M}_t^{\lambda,N})^2 = \abr{\ol{M}^{\lambda,N}}_t + 2 \int_0^t \ol{M}_{s-}^{\lambda,N} \odif{\ol{M}_s^{\lambda,N}}
  % \end{equation}
  % Note that \((\ol{M}_s^{\lambda,N})_{s\in[0,T]}\) is càdlàg by \zcref{eq:M-def} and adapted, so \((\ol{M}_{s-}^{\lambda,N})_{s\in[0,T]}\) is left-continuous and adapted.
  % This means that it is predictable (see e.g. Definition 8.3 \cite{klebanerIntroductionStochasticCalculus2012}), so as \((\ol{M}_s^{\lambda,N})_{s\in[0,T]}\) is bounded the stochastic integral is well-defined.
